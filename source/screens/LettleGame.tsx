...
import {LetterGameScreenProps} from '~/navigation/types';
...
import useSecretLetter from '~/store/useSecretLetter';

const {width} = Dimensions.get('window');

const LettleGame: React.FC<LetterGameScreenProps> = ({}) => {
  const maxAttempts = 6;
  const wordLength = 5;
  const category = 'GENERAL';
  const difficulty = 'easy';
  const gameType = 'RANDOM';
  ...
  const {evaluateGuess, secretWord, generateSecretLetter, aboutWord} =
    useSecretLetter(wordLength);
  ...
  const navigation = useNavigation<LetterGameScreenProps>();
  ...
  const resetGame = useCallback(() => {
    ...
    setScore(0);
    global.gc?.();
  }, [wordLength, maxAttempts, reset, generateSecretLetter, setScore]);
  ...
  const handleSubmit = useCallback(() => {
    if (
      ...
      dispatch({
        ...
        correctLetters: gameState.correctLetters, // No change needed to correctLetters
      });

      // Check if the secret letter was found (one green letter)
      const foundSecretLetter = correctness.some(c => c === 'correct');

      if (foundSecretLetter) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.content]}>
        ...
        />
        <GameTypeIndicator title={'מצא את האות'} />
